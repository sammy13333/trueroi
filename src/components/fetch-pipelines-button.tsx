...
export function FetchPipelinesButton({ clientId }: { clientId: string }) {
  ...
  return <div><button onClick={fetchPipelines} disabled={busy} className="rounded-md border border-[#1d5870] bg-[#0a2430] px-3 py-2 text-xs text-[#71d8ef] disabled:opacity-50">{busy ? "Fetching pipelines…" : "Fetch GHL pipelines"}</button>{message && <p className="mt-2 text-xs text-zinc-500">{message}</p>}</div>;
}
