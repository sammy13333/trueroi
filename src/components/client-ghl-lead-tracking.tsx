...
export async function ClientGhlLeadTracking({ requestedClientId, searchParams }: { requestedClientId?: string | string[]; searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  if (crmId && (crmLevel === "CAMPAIGN" || crmLevel === "ADSET" || crmLevel === "AD")) {
    ...
    const leads = await prisma.clientCrmLead.findMany({
      ...
      select: {
        ghlContactId: true, ghlOpportunityId: true, leadCreatedAt: true, pipelineGhlId: true, pipelineStageGhlId: true, pipelineStageName: true,
...
function CrmLeadDrilldown({
  ...
  bookedOnly: boolean;
  leads: Array<{
    ghlContactId: string; ghlOpportunityId: string | null; leadCreatedAt: Date | null; pipelineGhlId: string | null; pipelineStageGhlId: string | null; pipelineStageName: string | null;
